...
def determine_community_set(graph, vertex):
	...
	Computes the communities of the vertex with the Fastgreedy approach.  
	"""
	vertex_communities = []
...
def nectar(graph, beta, vertex_ID):
	...
	# TODO:  Edit code in this block below
	# TODO:  Fix the output "edge_betweeness_output" to deal with dendogram output
	...
	# Convert membership vector into VertexCluster object --> match output of community_multilevel function (Louvian method)
	communities = VertexClustering(modified_graph, membership=my_membership)

	# TODO: edit code in this block above
	# print("\nHere's some graph cluster info")
	# print("The entire graph has modularity")
	# print(louvian_output.modularity)
	# print("The clusters are")
	# print(communities)
	modified_clusters = communities.subgraphs() # breaks up the louvian output into the individual sub-graphs (aka, the clusters)
	cluster_membership = communities.membership # an array of all vertices' cluster_IDs; corresponds to pos' in modified_graph
...
def outer_nectar(graph, beta):
	...
	while (max_iter > 0 or stable_nodes < vertex_count ):
		print("Iterating outer MODIFIED nectar portion")
		stable_nodes = 1
